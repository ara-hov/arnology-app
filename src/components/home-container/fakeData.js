import NodeJsIcon from '../icon-containers/NodeJsIcon';
import PHPIcon from '../icon-containers/PHPIcon';

export const technologies = [
  {
    id: 1,
    title: 'Web development technologies we use',
    desc: [
      {
        id: '1a',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1b',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
      {
        id: '1c',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1d',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
      {
        id: '1e',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1f',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
      {
        id: '1g',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1h',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
    ],
  },
  {
    id: 2,
    title: 'Mobile development technologies we use',
    desc: [
      {
        id: '1a',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1b',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
      {
        id: '1c',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1d',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
      {
        id: '1e',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1f',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
      {
        id: '1g',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1h',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
    ],
  },
  {
    id: 3,
    title: 'UI/UX Product Design',
    desc: [
      {
        id: '1a',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1b',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
      {
        id: '1c',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1d',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
      {
        id: '1e',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1f',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
      {
        id: '1g',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1h',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
    ],
  },
  {
    id: 4,
    title: 'There can be your advertisment',
    desc: [
      {
        id: '1a',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1b',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
      {
        id: '1c',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1d',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
      {
        id: '1e',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1f',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
      {
        id: '1g',
        imgSrc: <PHPIcon />,
        info: 'PHP',
      },
      {
        id: '1h',
        imgSrc: <NodeJsIcon />,
        info: 'Nodejs',
      },
    ],
  },
];

export const testimonials = [
  {
    id: 1,
    info: '“The Arnology engineers are excellent and had tons of experience building apps. We felt like a real team from day one.”',
    name: 'SAUL GOODMAN',
    profession: 'FORMER DIRECTOR OF ENGINEERING AT FACEBOOK AI RESEARCH',
  },
  {
    id: 2,
    info: '“The Arnology engineers are excellent and had tons of experience building apps. We felt like a real team from day one.”',
    name: 'SAUL GOODMAN',
    profession: 'FORMER DIRECTOR OF ENGINEERING AT FACEBOOK AI RESEARCH',
  },
  {
    id: 3,
    info: '“The Arnology engineers are excellent and had tons of experience building apps. We felt like a real team from day one.”',
    name: 'SAUL GOODMAN',
    profession: 'FORMER DIRECTOR OF ENGINEERING AT FACEBOOK AI RESEARCH',
  },
];
